...
export default function CategoriesQuickAdd() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setParents(items.filter((x) => !x.parent_id));
    })();
  }, []);
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const res = await fetch("/.netlify/functions/categories", {
        ...
        body: JSON.stringify({ slug, label, parent_id: parentId || null, order_index: 0 }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed");

      setNote(`Saved: ${data.item.label}`);
      setLabel(""); setSlug(""); setParentId("");
    } catch (err) {
  ...
  return (
    <div className="max-w-lg p-6">
      <h1 className="text-xl font-semibold mb-4">Categories — Quick Add</h1>

      <form className="space-y-3" onSubmit={onSubmit}>
        <div>
          <label className="text-sm">Label</label>
          <input className="input input-bordered w-full" value={label} onChange={(e) => setLabel(e.target.value)} required />
        </div>

        <div>
          <label className="text-sm">Slug</label>
          <input className="input input-bordered w-full" value={slug} onChange={(e) => setSlug(e.target.value)} required />
        </div>
