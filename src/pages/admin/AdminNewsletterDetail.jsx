// src/pages/admin/AdminNewsletterDetail.jsx
...
export default function AdminNewsletterDetail() {
  ...
  const fetchCampaignLogs = async () => {
    const { data, error } = await supabase
      ...
      .select(
        "id, user_id, status, error, sent_at, opened, clicked, unsubscribed, opened_at, clicked_at, unsubscribed_at"
      )
      .eq("campaign_id", campaignId)
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `newsletter_campaign_${campaignId}.csv`;
    a.click();
  ...
  const exportJSON = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `newsletter_campaign_${campaignId}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  if (loading) {
    return <div className="p-6">Loading campaign logs…</div>;
  }

  return (
    <div className="p-6 max-w-7xl mx-auto dark:bg-gray-900 dark:text-gray-100 min-h-screen transition-colors">
      {/* Header */}
        ...
        <div>
          ...
          {campaign && (
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              <strong>{campaign.subject}</strong> — Sent{" "}
              {campaign.last_sent
                ? new Date(campaign.last_sent).toLocaleString("id-ID")
                : "-"}
            </p>
          )}
        </div>
          ...
          <tbody>
            {logs.map((l) => (
              ...
              >
                ...
                <td className="p-2 border">
                  {l.opened ? (
                    <span className="px-2 py-1 text-xs rounded bg-green-100 text-green-700">
                      ✅ {new Date(l.opened_at).toLocaleString("id-ID")}
                    </span>
                  ) : (
                    "-"
                  )}
                ...
                <td className="p-2 border">
                  {l.clicked ? (
                    <span className="px-2 py-1 text-xs rounded bg-blue-100 text-blue-700">
                      🔗 {new Date(l.clicked_at).toLocaleString("id-ID")}
                    </span>
                  ) : (
                    "-"
                  )}
                ...
                <td className="p-2 border">
                  {l.unsubscribed ? (
                    <span className="px-2 py-1 text-xs rounded bg-red-100 text-red-700">
                      🚫 {new Date(l.unsubscribed_at).toLocaleString("id-ID")}
                    </span>
                  ) : (
                    "-"
                  )}
                ...
                <td className="p-2 border">
                  {l.sent_at
                    ? new Date(l.sent_at).toLocaleString("id-ID")
                    : "-"}
                </td>
              </tr>
            ))}
